...
func main() {
	...
	sqlDepot, err := SqlDepot(*flDSN)
	if err != nil {
		lginfo.Log("err", err)
		os.Exit(1)
	}

	crt, key, err := sqlDepot.CreateOrLoadCA([]byte(*flCAPass), 10, "ca", "scep", "US")
	if err != nil {
		lginfo.Log("err", err)
		os.Exit(1)
	}
	...
	if *flChallenge == "" {
		signer = challenge.Middleware(sqlDepot, signer)
	} else {
	...
	mux.Handle("/scep", h)
	mux.Handle("/challenge", basicAuth(ChallengeHandlerFunc(sqlDepot, lginfo), "api", *flAPIKey, "SCEP Challenge"))
